...
Generated from: ('message_define.xml',)
...
MAVLINK_MSG_ID_SPEED = 1
MAVLINK_MSG_ID_HEARTBEAT = 2


class MAVLink_speed_message(MAVLink_message):
    """
    机器人的速度
    """

    id = MAVLINK_MSG_ID_SPEED
    msgname = "SPEED"
    fieldnames = ["vx", "vy", "vw"]
    ordered_fieldnames = ["vx", "vy", "vw"]
    fieldtypes = ["double", "double", "double"]
    fielddisplays_by_name: Dict[str, str] = {}
    fieldenums_by_name: Dict[str, str] = {}
    fieldunits_by_name: Dict[str, str] = {}
    native_format = bytearray(b"<ddd")
    orders = [0, 1, 2]
    lengths = [1, 1, 1]
    array_lengths = [0, 0, 0]
    crc_extra = 51
    unpacker = struct.Struct("<ddd")
    instance_field = None
    instance_offset = -1

    def __init__(self, vx: float, vy: float, vw: float):
        MAVLink_message.__init__(self, MAVLink_speed_message.id, MAVLink_speed_message.msgname)
        self._fieldnames = MAVLink_speed_message.fieldnames
        self._instance_field = MAVLink_speed_message.instance_field
        self._instance_offset = MAVLink_speed_message.instance_offset
        self.vx = vx
        self.vy = vy
        self.vw = vw

    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.vx, self.vy, self.vw), force_mavlink1=force_mavlink1)


# Define name on the class for backwards compatibility (it is now msgname).
# Done with setattr to hide the class variable from mypy.
setattr(MAVLink_speed_message, "name", mavlink_msg_deprecated_name_property())
...
class MAVLink_heartbeat_message(MAVLink_message):
    ...
    msgname = "HEARTBEAT"
    fieldnames = ["status", "time"]
    ordered_fieldnames = ["time", "status"]
    fieldtypes = ["uint8_t", "uint64_t"]
    fielddisplays_by_name: Dict[str, str] = {}
    fieldenums_by_name: Dict[str, str] = {}
    fieldunits_by_name: Dict[str, str] = {}
    native_format = bytearray(b"<QB")
    orders = [1, 0]
    lengths = [1, 1]
    array_lengths = [0, 0]
    crc_extra = 112
    unpacker = struct.Struct("<QB")
    instance_field = None
    instance_offset = -1

    def __init__(self, status: int, time: int):
    ...
    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.time, self.status), force_mavlink1=force_mavlink1)
...
mavlink_map: Dict[int, Type[MAVLink_message]] = {
    MAVLINK_MSG_ID_SPEED: MAVLink_speed_message,
...
class MAVLink(object):
    ...
    def speed_encode(self, vx: float, vy: float, vw: float) -> MAVLink_speed_message:
        """
        机器人的速度

        vx                        : x 方向速度 (type:double)
        vy                        : y 方向速度 (type:double)
        vw                        : w 方向角速度 (type:double)

        """
        return MAVLink_speed_message(vx, vy, vw)

    def speed_send(self, vx: float, vy: float, vw: float, force_mavlink1: bool = False) -> None:
        """
        机器人的速度
        ...
        """
        self.send(self.speed_encode(vx, vy, vw), force_mavlink1=force_mavlink1)

    def heartbeat_encode(self, status: int, time: int) -> MAVLink_heartbeat_message:
        """
        心跳包

        status                    : 状态 (type:uint8_t)
        time                      : 启动时间 (type:uint64_t)

        """
        return MAVLink_heartbeat_message(status, time)

    def heartbeat_send(self, status: int, time: int, force_mavlink1: bool = False) -> None:
        """
        心跳包
        ...
        """
        self.send(self.heartbeat_encode(status, time), force_mavlink1=force_mavlink1)
