...
{
    ((Swheel_t*)this)->current_pos= -hDJI[((Swheel_t*)this)->id].Calculate.RotorAngle_all/hDJI[((Swheel_t*)this)->id].reductionRate*PI/180.f/((Swheel_t*)this)->rotation_reduction;
    // = fmod(range,2*M_PI);
}
/**
 * @brief 内部函数，转向速度设置
 ...
 */
void mDJI_RspeedServo(void *this, const double speed)
{
    // speedServo(speed*hDJI[((Swheel_t*)this)->id].reductionRate*((Swheel_t*)this)->rotation_reduction,&hDJI[((Swheel_t*)this)->id]);
    speedpid[((Swheel_t*)this)->id].setpoint = -speed*((Swheel_t*)this)->rotation_reduction;
    PID_update(&speedpid[((Swheel_t*)this)->id],hDJI[((Swheel_t*)this)->id].AxisData.AxisVelocity);
}
/**
 * @brief 内部函数，转向位置设置
 * @param pos 轮子转过的总弧度，单位rad
 */
void mDJI_RposServo(void *this, const double pos)
{
    // positionServo(pos*180.f/PI*((Swheel_t*)this)->rotation_reduction,&hDJI[((Swheel_t*)this)->id]);

    pospid[((Swheel_t*)this)->id].setpoint= -pos*180.f/PI*((Swheel_t*)this)->rotation_reduction;
    PID_update(&pospid[((Swheel_t*)this)->id],hDJI[((Swheel_t*)this)->id].AxisData.AxisAngle_inDegree);
    speedpid[((Swheel_t*)this)->id].setpoint = pospid[((Swheel_t*)this)->id].output;
    PID_update(&speedpid[((Swheel_t*)this)->id],hDJI[((Swheel_t*)this)->id].AxisData.AxisVelocity);
...
void vesc_MspeedServo(void *this, const double speed)
{
    double erpm = 840*speed/STEERING_WHEEL_DIAMETER/PI;//n(rpm)=60f=60v/(pi*D) 840=14*60
...
void Swheel_init(
    ...
    double rotation_reduction,// 转向齿轮减速比（大比小）
    double correcting_speed,
...
static inline uint8_t _Swheel_correcting(Swheel_t *this)
{
    ...
    if(this->correcting_stage==0)//开始转动
    {
    ...
    else if(this->correcting_stage>0&&this->correcting_stage<1000)//反向转动
    {
        if(this->_light_switch_flag)
        {
...
void steeringWheel_executor(Swheel_t *this)
{
    ...
    {
        uint8_t not_aimed = Swheel_aim(this);
        this->RposServo(this, this->target_pos);
        if(not_aimed)
        {
            this->state=AIMMING;
            this->MspeedServo(this, 0);
        }else {
...
uint8_t Swheel_aim(Swheel_t *this)
{
    ...
    // else
    //     return 1;
    // double d_err = this->target_direction - this->direction;
    // if(d_err>PI)
    //     d_err-=2*PI;
    // else if(d_err<-PI)
    //     d_err+=2*PI;
    // this->RposServo(this, d_err+this->rotation_pos);
    double err;
    double speed = sqrt(this->target_vxy[0]*this->target_vxy[0]+this->target_vxy[1]*this->target_vxy[1]);
    if(this->target_vxy[0]==0)
    {
        err =PI/2;    
        this->target_main_speed = this->target_vxy[1];
    }
    else if (this->target_vxy[0]>0) {
        err =atan2(this->target_vxy[1],this->target_vxy[0]);
        this->target_main_speed = speed;
    }
    else  {
        err =atan2(-this->target_vxy[1],-this->target_vxy[0]);
        this->target_main_speed = -speed;
    }
    this->target_pos = err+this->current_pos;
    if(err>-0.5&&err<0.5)
        return 0;
    else
        return 1;
