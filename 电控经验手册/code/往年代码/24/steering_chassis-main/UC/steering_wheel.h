...
typedef void (*r_speedServo_func)(void *this, const double speed);
typedef void (*r_posServo_func)(void *this, const double pos);
typedef void (*r_reset_func)(void *this);
typedef void (*r_stop_func)(void *this);
typedef void (*m_speedServo_func)(void *this, const double speed);
...
typedef struct {
    ...
    double correcting_speed; // 校准速度，单位rad/s
    ...
    double rotation_speed; // 转向速度，单位rad/s
    double target_pos;
    double target_main_speed;
    double direction; // 方向，弧度制，0为正前方，逆时针为正，范围-2pi到2pi
    double current_pos; // 弧度制，0为正前方，逆时针为正
    double target_vxy[2]; // 目标速度，单位m/s
...
//    void Swheel_mMotor_speedServo(Swheel_t *this, const double speed);
