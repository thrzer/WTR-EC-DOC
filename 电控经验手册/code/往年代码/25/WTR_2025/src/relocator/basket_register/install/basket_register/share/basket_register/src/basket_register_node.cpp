...
public:
    BasketRegister(const std::string name) : Node(name)
    {
        ...
        lidar_sub_ = create_subscription<sensor_msgs::msg::PointCloud2>("/livox/lidar_PointCloud2", qos, std::bind(&BasketRegister::lidar_callback, this, std::placeholders::_1));
...
private:
    ...
    void publish_marker(const Eigen::Matrix4f& transform)
    {
        ...
        marker.header.frame_id = "odom"; // 以 odom 为参考系
