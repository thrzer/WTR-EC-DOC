...
/**
 * @brief 遥控器更新回调函数，在HAL_UART_RxCpltCallback(位于callback.c)中调用
 * @param msg 遥控器信息结构体指针
 */
...
void rmctl_decode(rmctl_msg_t* this)
{
    ...
    osDelay(1000);
    const float maxerror = 0.3f;
    float right_x_zeropoint = 0, right_y_zeropoint = 0, left_x_zeropoint = 0, left_y_zeropoint = 0;
    do//校正摇杆偏移
    {
        right_x_zeropoint             = ReadJoystickRight_x(&msg_joystick_air);
        right_y_zeropoint             = ReadJoystickRight_y(&msg_joystick_air);
        left_x_zeropoint              = ReadJoystickLeft_x(&msg_joystick_air);
        left_y_zeropoint              = ReadJoystickLeft_y(&msg_joystick_air);
        osDelay(5);
    }
    while(right_x_zeropoint > maxerror || right_y_zeropoint > maxerror ||
          left_x_zeropoint > maxerror || left_y_zeropoint > maxerror);
    osDelay(5);
    for (;;) {
        ...
        this->right_x             = ReadJoystickRight_x(&msg_joystick_air) - right_x_zeropoint;
        this->right_y             = ReadJoystickRight_y(&msg_joystick_air) - right_y_zeropoint;
        this->left_x              = ReadJoystickLeft_x(&msg_joystick_air) - left_x_zeropoint;
        this->left_y              = ReadJoystickLeft_y(&msg_joystick_air) - left_y_zeropoint;
        this->left_knob           = ReadJoystickKnobsLeft(&msg_joystick_air);
