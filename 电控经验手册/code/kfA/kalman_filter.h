...
typedef struct kf_t
{
    float *FilteredValue;       //滤波后的状态估计值，是卡尔曼滤波的输出，长度为状态向量的维度（xhatSize）
    float *MeasuredVector;      //观测量向量（z），存储传感器测量值
    float *ControlVector;       //控制输入向量（u），用于描述外部控制信号对系统的影响（可选，维度为 uSize）

    uint8_t xhatSize;           //状态向量的维度，表示需要估计的状态数量，例如位置、速度、加速度等
    uint8_t uSize;              //控制向量的维度，表示外部控制输入的数量
    uint8_t zSize;              //测量向量的维度，表示传感器测量值的数量

    uint8_t UseAutoAdjustment;  //是否启用自动调整模式，驱动后动态调整H和R
    uint8_t MeasurementValidNum;//有效测量值的数量，用于在自动调整时动态计算有效测量数据

    uint8_t *MeasurementMap;      // 测量值与状态的关系 how measurement relates to the state
