...
int extract_data(MOTOR_recv *motor_r)
{
    if(motor_r->motor_recv_data.CRC16 !=
		...
		{
        motor_r->motor_id = motor_r->motor_recv_data.mode.id;// 提取电机ID
        motor_r->mode = motor_r->motor_recv_data.mode.status;// 提取电机模式
        motor_r->Temp = motor_r->motor_recv_data.fbk.temp; // 提取电机温度
        motor_r->MError = motor_r->motor_recv_data.fbk.MError;// 提取电机错误状态
        motor_r->W = ((float)motor_r->motor_recv_data.fbk.speed/256)*6.2832f ;// 反馈速度
        motor_r->T = ((float)motor_r->motor_recv_data.fbk.torque) / 256; // 反馈扭矩
        motor_r->Pos = 6.2832f*((float)motor_r->motor_recv_data.fbk.pos) / 32768;// 反馈位置
				motor_r->footForce = motor_r->motor_recv_data.fbk.force;// 反馈力传感器数据
				motor_r->correct = 1;// 数据提取成功
        return motor_r->correct;
    }
}
/*
