...
HAL_StatusTypeDef Unitree_init(UnitreeMotor *MotorInstance,UART_HandleTypeDef *usartx, uint8_t id)
{
    ...
    MotorInstance->m_data.id=id;
    MotorInstance->m_data.mode=1;
    MotorInstance->m_data.T=0;
    MotorInstance->m_data.W=0;
    MotorInstance->m_data.Pos=0;
    MotorInstance->m_data.K_P=0;
    MotorInstance->m_data.K_W=0;
...
void Unitree_ChangeState(UnitreeMotor *MotorInstance, unsigned char motor_id,
                                          ...
                                          float Pos, float K_P, float K_W)
{
    MotorInstance->m_data.id = motor_id;
    MotorInstance->m_data.mode = mode;
    MotorInstance->m_data.T = T;
    MotorInstance->m_data.W = W;
    MotorInstance->m_data.Pos = Pos;
    MotorInstance->m_data.K_P = K_P;
    MotorInstance->m_data.K_W = K_W;
}
