 ...
 * @LastEditTime: 2024-05-02 12:43
 * @LastEditors: Liam
 * @Description: 添加vesc 刹车控制
...
void assert_param_brake_current(float *current){
	if( fabsf(*current) > wtrcfg_VESC_COMMAND_BRAKE_CURRENT_MAX)
		*current = *current > 0 ? wtrcfg_VESC_COMMAND_BRAKE_CURRENT_MAX : - wtrcfg_VESC_COMMAND_BRAKE_CURRENT_MAX ;
}
...
HAL_StatusTypeDef can_send_command(VESC_t *hvesc, const CAN_PACKET_ID id, float value,int all){
	...
	switch( id )
	{
		...
		case CAN_PACKET_SET_CURRENT_BRAKE:{
			assert_param_brake_current(&value);
			data = (int32_t)(value * 1e3f ) ;
			txbuf[0] = data >> 24 ;
			txbuf[1] = data >> 16 ;
			txbuf[2] = data >> 8 ;
